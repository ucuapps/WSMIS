...
with open(os.path.join(os.path.dirname(__file__), 'config', 'eval.yaml')) as config_file:
...
train_ds = PascalSegmentationDataset(train_config['train']['path'], transform=transform, image_set='train',
                                     masks_folder='out_masks/cam', return_original_mask=True)
val_ds = PascalSegmentationDataset(train_config['val']['path'], transform=transform, image_set='validation',
                                   masks_folder='../VOCdevkit/VOC2012/SegmentationClass', return_original_mask=True)
