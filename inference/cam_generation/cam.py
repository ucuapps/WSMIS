...
class ActivationExtractor:
    ...
    def __init__(self, config, train_dl, val_dl, device):
        ...
        self.maps_weights = self.model.linear.weight  # shape: [num_classes, K] K - number of kernel filters
    ...
    def _run(self, dl, prefix='train'):
        for X, y, image_names in tqdm.tqdm(dl):
            ...
            for name, activation_maps, classes in zip(image_names, maps, y):
                self._process_single_image(activation_maps, classes, name, (height, width), prefix=prefix)
    ...
    def _process_single_image(self, activation_maps, classes, name, size, prefix):
        ...
        threshold = (seg_maps_max.max() - seg_maps_max.min()) * self.config['background_threshold'] + seg_maps_max.min()
