...
class ActivationExtractor:
    ...
    def _run(self, dl, prefix='train'):
        ...
        for X, y, image_names, shapes in tqdm.tqdm(dl):
            ...
            for image, name, activation_maps, classes, shape, in zip(X, image_names, self.maps, y, shapes):
                self._process_single_image(image, activation_maps, classes, name, (height, width), shape, prefix=prefix)
    ...
    def _process_single_image(self, image, activation_maps, classes, name, size, original_shape, prefix):
        ...
        seg_maps_max = cv2.resize(seg_maps_max,
                                  (original_shape[1].item(), original_shape[0].item()), interpolation=cv2.INTER_NEAREST)
    ...
    def grub_cut_mask(image, seg_map, label):
        ...
        mask, _, _ = cv2.grabCut(image, new_mask, None, bgdModel, fgdModel, 5,
