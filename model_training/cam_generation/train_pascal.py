...
with open(os.path.join(os.path.dirname(__file__), 'config', 'pascal.yaml')) as config_file:
    config = yaml.full_load(config_file)

train_transform = get_transforms(config['train']['transform'])
val_transform = get_transforms(config['val']['transform'])

train_ds = PascalClassificationDataset(config['train']['path'], transform=train_transform, image_set='train')
val_ds = PascalClassificationDataset(config['val']['path'], transform=val_transform, image_set='validation')
