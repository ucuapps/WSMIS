...
with open(os.path.join(os.path.dirname(__file__), 'config', 'finetune_imagenet.yaml')) as config_file:
    config = yaml.full_load(config_file)

train_transform = get_transforms(config['train']['transform'])
val_transform = get_transforms(config['val']['transform'])

train_ds = ImageNetMLC(config['train']['path'], transform=train_transform)
val_ds = ImageNetMLC(config['val']['path'], transform=val_transform)
