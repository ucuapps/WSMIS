...
with open(os.path.join(os.path.dirname(__file__), 'config', 'pascal_crf.yaml')) as config_file:
    config = yaml.full_load(config_file)

train_transform = get_transforms(config['train']['transform'])
val_transform = get_transforms(config['val']['transform'])

train_ds = PascalCRFSegmentationDataset(config['train']['path'], transform=train_transform, image_set='train',
                                        masks_folder=config['train']['masks'], scale_factor=config['crf']['scale_factor'])
val_ds = PascalCRFSegmentationDataset(config['val']['path'], transform=val_transform, image_set='validation',
                                      masks_folder=config['val']['masks'], scale_factor=config['crf']['scale_factor'])
