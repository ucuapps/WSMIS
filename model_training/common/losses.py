...
class IALLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        neg_loss = -torch.log(torch.exp(-torch.max(y_pred[neg], scalar.expand_as(y_pred[neg]))) + 1e-8) \
...
def get_loss(loss_config):
    ...
    if loss_name == 'categorical_cross_entropy':
        ...
        return nn.BCEWithLogitsLoss(reduction='none', pos_weight=torch.tensor(loss_config.get('pos_weight')))
