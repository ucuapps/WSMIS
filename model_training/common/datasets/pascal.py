...
__all__ = ['PascalSegmentationDataset', 'PascalClassificationDataset', 'PascalCRFSegmentationDataset']


class PascalSegmentationDataset(torch.utils.data.Dataset):
    def __init__(self, images_list_path, transform, verbose=True, image_set='train',
                 masks_folder='segmentation', return_original_mask=False):
        ...
        with open(images_list_path, 'r') as fp:
            ...
            if image_set == 'train':
                self.images_list = list(map(lambda x: x['image'], annotations['train'])) + \
...
class PascalClassificationDataset(torch.utils.data.Dataset):
    def __init__(self, images_list_path, transform, verbose=True, image_set='train',
                 return_name=False, return_size=False):
        self.images_list_path = images_list_path
        self.transform = transform
        self.verbose = verbose
        ...
        with open(images_list_path, 'r') as fp:
            ...
            if image_set == 'train':
                self.images_list = list(map(lambda x: x, self.annotations[image_set]['train'])) + \
...
class PascalCRFSegmentationDataset(torch.utils.data.Dataset):
    def __init__(self, images_list_path, transform, image_set='train',
                 masks_folder='segmentation', scale_factor=8):
        self.images_list_path = images_list_path
        self.transform = transform
    ...
    def __getitem__(self, idx):
        ...
        return_values.append(
            cv2.resize(image, (w // self.scale_factor, h // self.scale_factor), interpolation=cv2.INTER_CUBIC)
        )
        return return_values
