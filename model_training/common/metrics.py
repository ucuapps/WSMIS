...
class MultiLabelAccuracy(Metric):
    ...
    def add(self, y_pred, y_true):
        ...
        self.correct_count += torch.sum((torch.sigmoid(y_pred) > self.THRESHOLD) == y_true, dim=0)
    ...
    def write_to_tensorboard(self, writer, epoch):
        ...
        ax.hist(np.arange(1, self.num_classes + 1), weights=self.accuracies.cpu().numpy(), bins=self.num_classes)
...
class BinaryF1(Metric):
    ...
    def get(self):
        self.positive_score = 2 * self.true_positives / (
                2 * self.true_positives + self.false_negatives + self.false_positives)
        self.negative_score = 2 * self.true_negatives / (
                2 * self.true_negatives + self.false_negatives + self.false_positives)
        self.precision = self.true_positives / (self.true_positives + self.false_positives)
        self.recall = self.true_positives / (self.true_positives + self.false_negatives)
        self.jaccard = self.true_positives / (self.true_positives + self.false_positives + self.false_negatives)
...
class MultiLabelF1(Metric):
    ...
    def get(self):
        self.scores = 2 * self.true_positives / (2 * self.true_positives + self.false_negatives + self.false_positives)
    ...
    def write_to_tensorboard(self, writer, epoch):
        fig, ax = plt.subplots(1, 1)
        ax.hist(np.arange(1, self.num_classes + 1), weights=self.scores.cpu().numpy(), bins=self.num_classes)
        ax.set_xlabel('Class ID')
        ax.set_ylabel('F1')
        writer.add_figure(self.NAME, fig, epoch)
