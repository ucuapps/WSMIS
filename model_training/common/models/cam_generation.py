...
class ResNet34(nn.Sequential):
    def __init__(self, num_classes):
        model_modules = list(models.resnet34(pretrained=False,
                                             num_classes=num_classes).named_children())
        # model_modules[0] = (
...
class ResNet50(nn.Sequential):
    def __init__(self, num_classes, replace_stride_with_dilation):
        model_modules = list(models.resnet50(pretrained=False, num_classes=num_classes,
                                             replace_stride_with_dilation=replace_stride_with_dilation).named_children())
        model_modules.insert(-1, ('flatten', Flatten()))
        super().__init__(OrderedDict(model_modules))
...
def resnet50(pretrained, num_classes, replace_stride_with_dilation, weights_path):
    model = ResNet50(num_classes if num_classes != 2 else 1, replace_stride_with_dilation)
